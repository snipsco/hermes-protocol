...
const char * last_error() {
    ...
    return * error; // we leak the string here but well crash just after so that's not that big a problem

}

#define check(hermes_expr) if( hermes_expr != SNIPS_RESULT_OK ) { printf("Assertion failed at %s:%i in function %s:\nFailed to execute %s\nError was %s\n", __FILE__, __LINE__, __func__, #hermes_expr, last_error()  ); exit(1); }
...
int main() {
    ...
    CStringArray ca_files = {.data = value_array,
                                   .size = sizeof(value_array) /
                                           sizeof(value_array[0])};

	const CMqttOptions options = {
    ...
    check(hermes_protocol_handler_new_mqtt_with_options(&truc, &options, "my user data"));
